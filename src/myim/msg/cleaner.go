...
func NewCleaner(cleaner int) *Cleaner {
	...
	c.root.prev = &c.root
	c.size = 0
	c.maps = make(map[string]*CleanData, cleaner)
	return c
}

func (c *Cleaner) PushFront(key string, expire time.Duration) {
	c.cLock.Lock()
...
func (c *Cleaner) Clean() (keys []string) {
	var (
		i int
		e *CleanData
	)
	keys = make([]string, 0, maxCleanNum)
	c.cLock.Lock()
